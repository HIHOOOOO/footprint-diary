...
- [x] 광고 실 ID 교체 (사업자 등록 → 토스 AIT 콘솔 광고 그룹 ID 발급 → ait-ad-test-banner-id 교체)
- [ ] 데이터 다운받기 버튼
- [ ] ConfirmDialog cancelButton 없을 때 레이아웃 실기기 확인 (TDS 컴포넌트 정상 렌더링 여부)
- [ ] 주소 재시도 최종 실패 시 UX 처리 ("위치 확인 중..." → "위치 확인 불가" 등 최종 상태 표시)
